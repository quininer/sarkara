...
use sarkara::aead::{ AeadCipher, OnlineAE, Encryption, Decryption };
use sarkara::aead::sparx256colm0::Sparx256Colm0;


fn test_aead<A: AeadCipher>() {
    let mut key = vec![0; A::KEY_LENGTH];
    let mut nonce = vec![0; A::NONCE_LENGTH];

    for i in 1..65 {
        let mut aad = vec![0; thread_rng().gen_range(0, 34)];

        let mut pt = vec![0; i];
        let mut ct = vec![0; pt.len() + A::TAG_LENGTH];
        ...
        for<'a> A: AeadCipher + OnlineAE<'a>
{
    ...
    for i in 1..65 {
        ...
        let a = thread::spawn(move || {
            let cipher = A::new(&key2);
            let mut process = cipher.encrypt(&nonce2, &aad2);

            let mut ctpos = 0;
            let mut ct = vec![0; pt2.len() + A::TAG_LENGTH];
            let mut buf = Vec::new();
